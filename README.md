...
1. **Launch** — the pitch, "New to nostr?", and for people who already have a key, a signer app or
   an `npub`/`nsec` field.
2. **Introduction** — three screens: an account is a key pair and nobody can reset it; posts live on
   relays and you pick which; this app contacts nothing until told to.
3. **Account or not** — "Create an account" and "Not for now", same visual weight. Both lead
   somewhere real; the second is a guest session with everything except publishing.
4. **Key backup**, if an account was made — the `nsec` in a selectable field (no clipboard write),
   an acknowledgement checkbox, and a note that Settings can show it again.
5. **A starting point** — a relay you have heard of, or an `npub`/`nprofile`, typed or scanned. This
   is the one question that cannot be answered for the user: with nothing approved, the app knows
   nowhere to look and nobody to look for.
6. **Relay consent**, whenever step 5 or a login needs relays that are not yet approved. An
   `nprofile` carries its own relay hints and those are offered as such; a bare `npub`, or a login
   with nothing approved, means the app has only its own defaults to suggest — so it lists them,
   says they are its guess rather than yours, and takes a relay of your own instead if you have one.

Onboarding owns the whole window while it is up: no tab bar is drawn around it. That is what keeps
the one-time key screen from being one mis-tap from gone, and it is why the steps are an
`OnboardingStep` sequence rather than `Screen` destinations the navigation bar can reach.
