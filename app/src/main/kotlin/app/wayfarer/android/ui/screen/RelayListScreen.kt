...
fun RelayListScreen(controller: AppController) {
    ...
    LazyColumn(
        ...
        item { StatusCard(state.loading, state.loaded, state.publishedAt, state.isSuggestion, state.edited) }
        ...
        item {
            OutlinedButton(onClick = { controller.go(Screen.Relays) }, modifier = Modifier.fillMaxWidth()) {
                Text("Open the other list — what this app may connect to")
            }
        }
    }
}
            ...
            when {
                ...
                isSuggestion ->
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        ...
                        Text(
                            "Last published at $publishedAt (unix time). Other people's apps read this to find you.",
                            style = MaterialTheme.typography.bodySmall,
                        )
                    }
            }
