...
fun OnboardingScreen(
    ...
    onCreate: () -> Unit,
    onLogin: (String) -> Unit,
    onLoginWithExternalSigner: () -> Unit,
    ...
    Column(
        ...
        if (externalSignerAvailable) {
            HorizontalDivider()
            OutlinedButton(
                onClick = onLoginWithExternalSigner,
                enabled = !busy,
            ...
            Text(
                "Signing happens in a separate app that holds your key (NIP-55). Wayfarer never sees it, " +
                    "and every signature is one you approve there.",
                style = MaterialTheme.typography.bodySmall,
            )
        }
